...
class TradingBot:
    ...
    async def _process_trading_signal(self, signal: Dict):
        ...
        try:
            ...
            if not risk_assessment['approved']:
                self.logger.info(f"Trade rejected by risk manager: {risk_assessment['reason']}")
                return
    ...
    async def _handle_filled_order(self, order: Dict, status: Dict):
        ...
        try:
            ...
            # Update portfolio
            await self.portfolio_manager.update_position(symbol, self.active_positions.get(symbol))
